...
class GmailService:
    def __init__(self,
        ...
        self.scopes = scopes
        logger.info("GmailService initialized (multi-user mode)")

    def _create_service_from_token(self, access_token: str) -> Any:
        """Create Gmail service using provided access token"""
        try:
            # 使用传入的 access_token 创建凭据
    ...
    def _get_user_email_from_token(self, access_token: str) -> str:
        """Get user email address using access token"""
        service = self._create_service_from_token(access_token)
        profile = service.users().getProfile(userId='me').execute()
        user_email = profile.get('emailAddress', '')
        return user_email
    
    async def send_email(self, access_token: str, recipient_id: str, subject: str, message: str) -> dict:
        """Creates and sends an email message using provided access token"""
        try:
            service = self._create_service_from_token(access_token)
            user_email = self._get_user_email_from_token(access_token)
    ...
    async def get_unread_emails(self, access_token: str) -> list[dict[str, str]]| str:
        """
        Retrieves unread messages from mailbox using access token.
    ...
    async def read_email(self, access_token: str, email_id: str) -> dict[str, str]| str:
        """Retrieves email contents including to, from, subject, and contents using access token."""
        try:
            service = self._create_service_from_token(access_token)
            ...
            await self.mark_email_as_read(access_token, email_id)

            return email_metadata
        except HttpError as error:
            return f"An HttpError occurred: {str(error)}"
        
    async def trash_email(self, access_token: str, email_id: str) -> str:
        """Moves email to trash given ID using access token."""
        try:
            service = self._create_service_from_token(access_token)
    ...
    async def mark_email_as_read(self, access_token: str, email_id: str) -> str:
        """Marks email as read given ID using access token."""
        try:
            service = self._create_service_from_token(access_token)
...
async def main(creds_file_path: str):
    ...
    gmail_service = GmailService(creds_file_path)
    server = Server("gmail")
    ...
    async def handle_list_tools() -> list[types.Tool]:
        return [
            types.Tool(
                name="send-email",
                description="""Sends email to recipient using user's access token. 
                ...
                inputSchema={
                    ...
                    "properties": {
                        "access_token": {
                            "type": "string",
                            "description": "OAuth 2.0 access token from Google Identity Services",
                        },
                        "recipient_id": {
                            "type": "string",
                    ...
                    "required": ["access_token", "recipient_id", "subject", "message"],
                },
            ),
            types.Tool(
                name="trash-email",
                description="""Moves email to trash using user's access token. 
                ...
                inputSchema={
                    ...
                    "required": ["access_token", "email_id"],
                },
            ),
            types.Tool(
                name="get-unread-emails",
                description="Retrieve unread emails using user's access token",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "access_token": {
            ...
            types.Tool(
                name="read-email",
                description="Retrieves given email content using user's access token",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "access_token": {
            ...
            types.Tool(
                ...
                description="Marks given email as read using user's access token",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "access_token": {
            ...
            types.Tool(
                name="open-email",
                description="Open email in browser (doesn't require access token)",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "email_id": {
    ...
    async def handle_call_tool(
        ...
        if name == "send-email":
            ...
            if not access_token:
                raise ValueError("Missing access_token parameter")
            ...
            send_response = await gmail_service.send_email(access_token, recipient, subject, message_content)
        ...
        if name == "get-unread-emails":
            ...
            unread_emails = await gmail_service.get_unread_emails(access_token)
        ...
        if name == "read-email":
            ...
            retrieved_email = await gmail_service.read_email(access_token, email_id)
        ...
        if name == "trash-email":
            ...
            msg = await gmail_service.trash_email(access_token, email_id)
            return [types.TextContent(type="text", text=str(msg))]
        if name == "mark-email-as-read":
            ...
            msg = await gmail_service.mark_email_as_read(access_token, email_id)
            return [types.TextContent(type="text", text=str(msg))]
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Gmail API MCP Server (Multi-user)')
    parser.add_argument('--creds-file-path',
                        required=True,
                       help='OAuth 2.0 credentials file path')
    
    args = parser.parse_args()
    asyncio.run(main(args.creds_file_path))